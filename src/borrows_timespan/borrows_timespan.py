...
def first_repayment(
    ...
    if len(df_repay_user) == 1:
        ...
        for i in range(len(df_borrow_user)):
            if repay[col_day] >= df_borrow_user.iloc[i][col_day]:  # ensure repayment is after borrow
                results.append({
                    ...
                    "Time_to_First_Repay": repay[col_day] - df_borrow_user.iloc[i][col_day]
                })
            else:
                results.append({
                    "user": df_borrow_user.iloc[i][col_user],
                ...
                if i == 0:
                    ...
                    if borrow_amount[i] >= repay_cum:
                        results.append({
                            ...
                            "Time_to_First_Repay": repay_day[index_repay] - borrow_day[i]
                        })
                        matched = True
                    else:
                        index_repay += 1
...
def last_repayment(
    ...
    for i in range(len(df_borrow_user)):
        ...
        while not matched and index_repay < repay_len:
            ...
            threshold = borrow_amount[i] if i == 0 else borrow_amount[i] + borrow_cumsum[i - 1]
...
def first_last_repayment(
    ...
    first_repay = first_repayment(
        df_borrow_user, df_repay_user,
        col_amount, col_cumsum, col_user, col_asset, col_day, col_block
    )
    ...
    cols_to_add = ["Last_Repay_Day", "Last_Repay_blockNumber", "Last_Repay_Amount", "Time_to_Last_Repay"]
...
def first_last_repayments_by_user(
    ...
    for asset in assets:
        ...
        df_borrow_user = borrow[borrow[col_asset] == asset][[col_user_borrow, col_day, col_asset, col_amount, col_block]].copy()
        df_borrow_user["Cumsum_amount"] = df_borrow_user[col_amount].cumsum()

        df_repay_user = repay[repay[col_asset] == asset][[col_user_repay, col_day, col_asset, col_amount, col_block]].copy()
        ...
        df_repay_match = first_last_repayment(
            df_borrow_user, df_repay_user,
            col_amount, "Cumsum_amount", col_user_borrow,
            col_asset, col_day, col_block
        )
        # Append to list
